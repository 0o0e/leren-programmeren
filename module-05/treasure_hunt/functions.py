...
def getEarnigs(profitGold:float, mainCharacter:dict, friends:list, investors:list) -> list:
    ...
    eindcash = round(begincash + friends_extramoney+ ((profitGold - sum(getInvestorsCuts(profitGold,interestingInvestors))) / len(fellowship)),2)
    earnings.append({
    ...
    for i in friends:
        if i['adventuring'] == True and i['shareWith'] == True:
            ...
            eindcash = round((begincash + ((profitGold - sum(getInvestorsCuts(profitGold,interestingInvestors))) / len(fellowship))) - 10, 2)
